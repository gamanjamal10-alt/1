...
const RetailerDashboard: React.FC = () => {
    ...
    const navItems = [
        { label: 'retailProducts', view: 'dashboard', icon: DashboardIcon },
        { label: 'myPurchases', view: 'orders', icon: CartIcon },
    ...
    return (
        <DashboardLayout navItems={navItems} activeView={activeView} setActiveView={setActiveView}>
             {!selectedProductId && <h2 className="text-3xl font-bold text-primary mb-6">{t('retailerControlPanel')}</h2>}
            {renderContent()}
        </DashboardLayout>
    );
};
