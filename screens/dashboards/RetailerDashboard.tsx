...
const RetailerDashboard: React.FC = () => {
    ...
    const navItems = [
        { label: 'Retail Products', view: 'dashboard', icon: DashboardIcon },
        { label: 'My Purchases', view: 'orders', icon: CartIcon },
    ...
    const renderContent = () => {
        ...
        switch (activeView) {
            ...
            case 'dashboard':
            default:
                return <BuyerDashboard role={UserRole.RETAILER} onSelectProduct={handleSelectProduct} />;
        }
    };

    return (
        <DashboardLayout navItems={navItems} activeView={activeView} setActiveView={setActiveView}>
             {!selectedProductId && <h2 className="text-3xl font-bold text-primary mb-6">Retailer Control Panel</h2>}
            {renderContent()}
        </DashboardLayout>
    );
};
